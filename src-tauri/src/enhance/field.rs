...
pub fn use_sort(mut config: Mapping) -> Mapping {
    // Karing-style diversion is an app-only extension. Compile it after all
    // subscription/merge/script processing and remove the private field before
    // the final Mihomo configuration is emitted.
