...
<p align="center">
  基于 Clash Verge Rev 的桌面增强分支，提供完整分流管理、程序/项目档案、连接与代理组联动，以及可审计的多平台发行流程。
</p>
...
本分支保留 Clash Verge Rev 的 Mihomo 桌面代理管理能力，并把规则、连接、程序/项目和代理组组织成同一套可追溯关系。
...
程序/项目档案用于解决 Mihomo 无法稳定返回进程信息时的分流问题。每个档案可保存：
...
保存档案后，软件会生成一个真实的 Mihomo 分流规则组，而不是只在界面里做标签。匹配优先级为：
...
规则页顶部显示程序项目关系，包含识别连接数、出口连接数、托管规则组和目标代理组。点击项目可直接打开完整编辑界面。

### 从代理组出发

项目指定代理组后，可从项目卡片跳转到该代理组。代理组页会显示：
