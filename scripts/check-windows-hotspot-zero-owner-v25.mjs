...
const paths = {
  ...
  hotspotWorkflow: '.github/workflows/karing-windows-hotspot-v25.yml',
}
...
for (const key of ['v22Compat', 'v23Compat']) {
  requireText(
    ...
    `${key} must delegate to the v25 lifecycle/data-plane gate`,
  )
}

for (const marker of [
  'check-windows-hotspot-zero-owner-v25.mjs',
  ...
  requireText('hotspotWorkflow', marker, `v25 workflow regression ${marker}`)
}

if (failures.length > 0) {
  console.error(
    'Windows Mobile Hotspot v25 lifecycle/data-plane safety gate failed:',
  )
...
console.log(
  '[通过] Windows x64/ARM64 编译与真实 ICS/network 单测执行被 v25 workflow 强制验证',
)

// Final same-SHA audit stamp: keep v25, frontend and release gates bound to one commit.
